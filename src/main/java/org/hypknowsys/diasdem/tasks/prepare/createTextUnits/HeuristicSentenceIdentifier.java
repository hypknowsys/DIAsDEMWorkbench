 ...
 * ('.') in abbrviations with asterisks ('*') and then splitting the text at the
...
public class HeuristicSentenceIdentifier implements TextUnitCreator {
  ...
  private Matcher TmpTextUnitsMatcher = null;
  private ArrayList TmpTextUnitsArray = null;
  private DIAsDEMtextUnit[] TmpResult = null; 
  ...
  public HeuristicSentenceIdentifier(String[] pAbbreviations,
  ...
  public DIAsDEMtextUnit[] createTextUnits(String pText) {
    ...
    // split text into text units at remaining full stops
    TmpTextUnitsMatcher = FullStopPattern.matcher(pText);
    TmpTextUnitsArray = new ArrayList();
    int beginIndex = 0;
    while (TmpTextUnitsMatcher.find()) {
      TmpTextUnitsArray.add(new DefaultDIAsDEMtextUnit(
      pText.substring(beginIndex, TmpTextUnitsMatcher.end()).trim(), 
      TmpTextUnitsArray.size(), beginIndex, TmpTextUnitsMatcher.end()));
      beginIndex = TmpTextUnitsMatcher.start() + 1;
      for (int i = TmpTextUnitsMatcher.start() + 1; i < pText.length(); i++) {
    ...
    if (beginIndex < pText.length()) {
      TmpTextUnitsArray.add(new DefaultDIAsDEMtextUnit(pText
      .substring(beginIndex).trim(), TmpTextUnitsArray.size(), 
      beginIndex, pText.length()));
    }
    
    if (TmpTextUnitsArray != null) {
      TmpResult = new DefaultDIAsDEMtextUnit[TmpTextUnitsArray.size()];
      for (int i = 0; i < TmpTextUnitsArray.size(); i++) {
        TmpResult[i]= (DefaultDIAsDEMtextUnit)TmpTextUnitsArray.get(i);
      }
    }
    else {
      TmpResult = new DefaultDIAsDEMtextUnit[1];
      TmpResult[0]= new DefaultDIAsDEMtextUnit("null", 0);
    }
    // replace stars with full stops to restore original abbreviations
    if (!KeepStars) {
      for (int i = 0; i < TmpResult.length; i++) {
        TmpMatcher = StarPattern.matcher(TmpResult[i].getContentsAsString());
        TmpResult[i].setContentsFromString(TmpMatcher.replaceAll("."));
      }
    }
  ...
  public static void main(String[] args) {
    ...
    HeuristicSentenceIdentifier sentenceCreator =
    new HeuristicSentenceIdentifier(abbreviations, regexSearch, 
    regexReplace, true);
