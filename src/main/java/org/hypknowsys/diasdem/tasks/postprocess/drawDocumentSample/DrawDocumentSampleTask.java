...
public class DrawDocumentSampleTask extends DiasdemScriptableNonBlockingTask {
  ...
  Project pProject, TaskParameter pParameter) {
    ...
    && parameter.getDtdFileName().length() > 9) {
      ...
      .endsWith(DIAsDEMguiPreferences.PRELIMINARY_DTD_FILE_EXTENSION)) {
        ...
        + "-file in the field 'Preliminary DTD File'! This\n"
  ...
  public void run() {
    ...
    try {
      DerivedDtd = new DefaultDIAsDEMpreliminaryDtd(
  ...
  private void createRandomFile() {
    ...
    sampleTextUnitsFile.setNextLine("# DIAsDEM Collection: " + Tools.
    shortenFileName(DiasdemCollection.getCollectionFileName(), 50));
    sampleTextUnitsFile.setNextLine("# Number of Documents: "
    + DiasdemCollection.getNumberOfDocuments());
    sampleTextUnitsFile.setNextLine("# Random Sample Size: "  + CastParameter
    .getRandomSampleSize() + " * " + DiasdemCollection.getNumberOfDocuments()
    + " = " + (int)sampleSize);
  ...
  private void applyRandomFile() {
    ...
    sampleDocsHtmlFile.setNextLine("<h3>Parameter Settings</h3><p>");
    ...
    sampleDocsHtmlFile.setNextLine("</p>");
