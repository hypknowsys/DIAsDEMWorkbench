...
public class TagDocumentsTask extends DiasdemScriptableNonBlockingTask {
  ...
  "org.hypknowsys.diasdem.tasks.postprocess.tagDocuments"
  ...
  Project pProject, TaskParameter pParameter) {
    ...
    .endsWith(DIAsDEMguiPreferences.PRELIMINARY_DTD_FILE_EXTENSION)) {
      ...
      "-file in the field 'Preliminary DTD File'!");
    }
  ...
  public void run() {
    ...
    while (DiasdemDocument != null) {
      ...
      i++) {
        ...
        && DiasdemTextUnit.getClusterLabel().equals("-")) {
          ...
          .getClusterLabel(), namedEntities, DerivedDtd.getMinAttributeRelSupport());       
          if (CastParameter.createTagByDocumentFile()) {
            // remember XML tags
    ...
    + DerivedDtd.getNumberOfTextUnits());
    ...
    * DerivedDtd.getNumberOfTextUnits();
    ...
    for (int i = 0; sortedRandoms.size() < (int)sampleSize; i++)
      sortedRandoms.add( new Integer(
      randomizer.nextInt( (int)DerivedDtd.getNumberOfTextUnits() + 1 ) + 1) );
