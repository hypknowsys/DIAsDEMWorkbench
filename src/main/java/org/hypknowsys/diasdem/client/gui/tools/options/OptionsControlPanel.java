...
public class OptionsControlPanel extends DiasdemActionsControlPanel {
  ...
  private KTextField TreeTaggerFile_Text = null;
  ...
  private KButtonPanel TreeTaggerFile_Button = null;
  ...
  public OptionsControlPanel(Server pDiasdemServer, Project pDiasdemProject, GuiClient pDiasdemGui, GuiClientPreferences pDiasdemGuiPreferences, String pFileName) {
  ...
  public void actionPerformed(ActionEvent e) {
    ...
    } else if (ActionCommand.equals("TreeTaggerFile_Button")) {
      CurrentDirectory = this.fileNameButtonClicked(
      TreeTaggerFile_Text, CurrentDirectory, "PROJECT_DIRECTORY", 
      "Select", KeyEvent.VK_S, null, "Select the TreeTagger Executable File");    
  ...
  public void createInputPanels() {  
    ...
    EditorFile_Text = new KTextField(
      DiasdemGuiPreferences.getProperty("EXTERNAL_EDITOR") );
    BrowserFile_Text = new KTextField(
      DiasdemGuiPreferences.getProperty("EXTERNAL_BROWSER") );
    XmlViewerFile_Text = new KTextField(
      DiasdemGuiPreferences.getProperty("EXTERNAL_XML_VIEWER") );
    TreeTaggerFile_Text = new KTextField(
      DiasdemGuiPreferences.getProperty("DEFAULT_TREE_TAGGER_COMMAND") );
    ...
    TreeTaggerFile_Button = new KButtonPanel(0, 0, 0, 0, 1, 
      KButtonPanel.HORIZONTAL_RIGHT);
    TreeTaggerFile_Button.addSingleButton("...", KeyEvent.VK_T, true, true, 
      "TreeTaggerFile_Button", this);    
    ...
    Applications_Panel.addBlankRow(0, 5, 11);
    Applications_Panel.addLabel("TreeTagger:", 0, 6, KeyEvent.VK_T,
      TreeTaggerFile_Button);
    Applications_Panel.addComponent(TreeTaggerFile_Text, 2, 6);
    Applications_Panel.addKButtonPanel(TreeTaggerFile_Button, 4, 6);
    ...
    for (int i = 0; i < PlafDescriptors.length; i++) {
      if (PlafClassNames[i].equals(DiasdemGuiPreferences
    ...
    if (DiasdemProject.getProjectFileName() != null
    && DiasdemProject.getProjectFileName().length() > 0) {      
  ...
  private void ok() {
      ...
      for (int i = 0; i < GuiPreferences_TableModel.getRowCount(); i++) {
        if ( ! GuiPreferences_TableModel.isValidValue(i) ) {
          JOptionPane.showMessageDialog(DiasdemGui.getJFrame(), "Input Error:\n\n" +
      ...
      if (commit) { 
        ...
        if (!PlafClassNames[selectedPlaf].equals(DiasdemGuiPreferences
        .getLookAndFeelClassName().trim())) {
          DiasdemGuiPreferences.setLookAndFeelClassName(
        ...
        DiasdemGuiPreferences = GuiPreferences_TableModel.commit();
        DiasdemGuiPreferences.setProperty("EXTERNAL_EDITOR", 
          EditorFile_Text.getText().trim() );
        DiasdemGuiPreferences.setProperty("EXTERNAL_BROWSER", 
          BrowserFile_Text.getText().trim() );
        DiasdemGuiPreferences.setProperty("EXTERNAL_XML_VIEWER", 
          XmlViewerFile_Text.getText().trim() );
        DiasdemGuiPreferences.setProperty("DEFAULT_TREE_TAGGER_COMMAND", 
          TreeTaggerFile_Text.getText().trim() );

        // check TASK_THREAD_PRIORITY
        int currentTaskThreadPriority = DiasdemGuiPreferences.getIntProperty(
        "TASK_THREAD_PRIORITY");
        if (currentTaskThreadPriority < Thread.MIN_PRIORITY ||
          currentTaskThreadPriority > Thread.MAX_PRIORITY)
          DiasdemGuiPreferences.setProperty("TASK_THREAD_PRIORITY",
          ( new Integer( Thread.NORM_PRIORITY ) ).toString() );
        DiasdemServer.setTaskThreadPriority(DiasdemGuiPreferences
        ...
        if (DiasdemProject.getProjectFileName() != null
        && DiasdemProject.getProjectFileName().length() > 0) {
          DiasdemProject = ProjectProperties_TableModel.commit();
