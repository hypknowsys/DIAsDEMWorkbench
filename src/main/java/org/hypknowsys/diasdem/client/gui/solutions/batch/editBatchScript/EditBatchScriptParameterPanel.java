...
import org.hypknowsys.diasdem.core.default21.DefaultDIAsDEMscriptTask;
...
public class EditBatchScriptParameterPanel extends DiasdemParameterPanel
implements TableModelListener, ListSelectionListener {
  ...
  public EditBatchScriptParameterPanel(Server pDiasdemServer, 
  ...
  public void tableChanged(TableModelEvent pEvent) {
    ...
    String columnName = DiasdemScript.getColumnName(column);
    Object data = DiasdemScript.getValueAt(row, column);
  ...
  public TaskParameter getTaskParameter() {
    ...
    DiasdemScript.setLabel(Label_Text.getText());
    DiasdemScript.setNotes(Notes_Text.getText());
  ...
  public void setTaskParameter(TaskParameter pTaskParameter) {
    ...
    DiasdemScript = parameter.getDiasdemScript(); 
    IsEnabled = true;
    SaveRequired = false;
    this.createParameterPanel();
  ...
  protected void createParameterPanel() {
    ...
    if (DiasdemScript != null) {
      Label_Text = new KTextField(DiasdemScript.getLabel(), 30, true);
      Label_Text.setCaretAtEnding();
      Status_Text = new KTextField(
      DiasdemScript.getStatusString(), 30, true, false);
      Status_Text.setCaretAtEnding();
      FileName_Text = new KTextField(
      DiasdemScript.getTransientFileName(), 30, true, false);
      FileName_Text.setCaretAtEnding();
      Notes_Text = new KScrollTextArea(DiasdemScript.getNotes(), true);
      StartTimeStamp_Text = new KTextField(
      DiasdemScript.getStartTimeStamp(), 30, true, false);
      StartTimeStamp_Text.setCaretAtEnding();
      EndTimeStamp_Text = new KTextField(
      DiasdemScript.getEndTimeStamp(), 30, true, false);
      EndTimeStamp_Text.setCaretAtEnding();
      Log_Text = new KScrollTextArea(DiasdemScript.getLog(), true, false);
    }
    else {
    ...
    "Optional input: Concise name of this DIAsDEM batch script");
    ...
    TasksButton_Panel.addNextButton("Delete",
  ...
  private void setComponentStatus() {
    ...
    if (DiasdemScript != null && IsEnabled) {
      ...
      enableAppend = true;    
      if (DiasdemScript.countScriptTasks() > 0 && SelectedTask >= 0) {
        enableInsert = true;
        enableEdit = true;
        enableReset = true;     
        enableDelete = true;        
      ...
      if ((DiasdemScript.getLog() != null 
      && DiasdemScript.getLog().length() > 0)
      || (DiasdemScript.getStartTimeStamp() != null
      && DiasdemScript.getStartTimeStamp().length() > 0)
      || (DiasdemScript.getEndTimeStamp() != null
      && DiasdemScript.getEndTimeStamp().length() > 0)) {
  ...
  private void append() {
    ...
    if (SelectedTask >= 0  && SelectedTask < DiasdemScript.countScriptTasks()) {
      int taskID = SelectedTask;
      DiasdemScript.appendScriptTask(new DefaultDIAsDEMscriptTask(
      Tools.getSystemDate(), "", null, ""), taskID);
      SelectedTask = Math.min(taskID + 1, DiasdemScript.countScriptTasks() - 1);
      Tasks_Table.setRowSelectionInterval(SelectedTask, SelectedTask);
    }
    else {
      DiasdemScript.addScriptTask(new DefaultDIAsDEMscriptTask(
      Tools.getSystemDate(), "", null, ""));     
      SelectedTask = Math.max(0, DiasdemScript.countScriptTasks() - 1);
      Tasks_Table.setRowSelectionInterval(SelectedTask, SelectedTask);
    }
    SaveRequired = true;
    this.setComponentStatus();
  ...
  private void insert() {
    ...
    if (SelectedTask >= 0 && SelectedTask < DiasdemScript.countScriptTasks()) {
      int taskID = SelectedTask;
      DiasdemScript.insertScriptTask(new DefaultDIAsDEMscriptTask(
      Tools.getSystemDate(), "", null, ""), taskID);
  ...
  private void edit() {
    ...
    if (SelectedTask >= 0 && SelectedTask < DiasdemScript.countScriptTasks()) {
      int taskID = SelectedTask;
      ScriptTask task = DiasdemScript.getScriptTask(taskID);
      ...
      parameter = (EditBatchScriptTaskParameter)DiasdemParameterDialog
      ...
      if (parameter != null) {      
        DiasdemScript.replaceScriptTask(parameter.getDiasdemScriptTask(),
        taskID);
        SaveRequired = true;
        this.setComponentStatus();  
      }
  ...
  private void resetTask() {
    ...
    if (SelectedTask >= 0 && SelectedTask < DiasdemScript.countScriptTasks()) {
      ...
      DiasdemGui.getJFrame(), "Warning: Do you really want\n"
      ...
      if (commit) {
        int taskID = SelectedTask;
        DiasdemScript.resetScriptTask(taskID);
        SaveRequired = true;
        this.setComponentStatus();
      }
    }
  ...
  private void delete() {
    ...
    if (SelectedTask >= 0 && SelectedTask < DiasdemScript.countScriptTasks()) {
      ...
      if (commit) {
        int taskID = SelectedTask;
        DiasdemScript.deleteScriptTask(taskID);
        if (taskID < DiasdemScript.countScriptTasks()) {
          SelectedTask = taskID;
          Tasks_Table.setRowSelectionInterval(SelectedTask, SelectedTask);
        }
        else if (DiasdemScript.countScriptTasks() > 0) {
  ...
  private void cut() {
    ...
    if (SelectedTask >= 0 && SelectedTask < DiasdemScript.countScriptTasks()) {
      ...
      if (commit) {
        int taskID = SelectedTask;
        if (DiasdemScript.getScriptTask(taskID).clone() != null) {
          ClipboardTask = (ScriptTask)DiasdemScript.getScriptTask(taskID)
          .clone();
          DiasdemScript.deleteScriptTask(taskID);
          if (taskID < DiasdemScript.countScriptTasks()) {
            SelectedTask = taskID;
  ...
  private void copy() {
    ...
    if (SelectedTask >= 0 && SelectedTask < DiasdemScript.countScriptTasks()) {
      int taskID = SelectedTask;
      if (DiasdemScript.getScriptTask(taskID).clone() != null) {
        ClipboardTask = (ScriptTask)DiasdemScript.getScriptTask(taskID).clone();
      }
      else {
        ClipboardTask = null;
      }     
  ...
  private void paste() {
    ...
    if (SelectedTask >= 0  && SelectedTask < DiasdemScript.countScriptTasks()) {
      int taskID = SelectedTask;
      DiasdemScript.appendScriptTask((ScriptTask)ClipboardTask.clone(), taskID);
      SelectedTask = Math.min(taskID + 1, DiasdemScript.countScriptTasks() - 1);
      Tasks_Table.setRowSelectionInterval(SelectedTask, SelectedTask);
    }
    else {
      DiasdemScript.addScriptTask((ScriptTask)ClipboardTask.clone());     
      SelectedTask = Math.max(0, DiasdemScript.countScriptTasks() - 1);
      Tasks_Table.setRowSelectionInterval(SelectedTask, SelectedTask);
    }
    SaveRequired = true;
