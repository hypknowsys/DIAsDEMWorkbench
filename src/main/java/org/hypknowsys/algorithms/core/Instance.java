...
/*
 *    This class is equivalent to weka.core.Instance.java (revision 1.15).
...
public class Instance implements Copyable, Serializable {
   ...
   *
   * @param instance another instance
   * @return true if the values of the given instance is 
   * equivalent to this instance's values
   * @author Karsten Winkler, kwinkler{at}hypKNOWsys{d0t}org
   */
  public boolean equalValues(Instance inst) {

    if (m_AttValues == null || inst.m_AttValues == null
    || m_AttValues.length != inst.m_AttValues.length) {
      return false;
    }
    for (int i = 0; i < m_AttValues.length; i++) {
      if (!Utils.eq(m_AttValues[i], inst.m_AttValues[i])) {
        return false;
      }
    }
  ...
  /**
   * Tests if the two instances are equivalent by checking both their
   * headers and their values.
   *
   * @param instance another instance
   * @return true if (i) the header of the given instance is 
   * equivalent to this instance's header and (ii) the values
   * of the given instance are equivalent to this instance's values 
   * @exception UnassignedDatasetException if instance doesn't have access to any
   * dataset
   * @author Karsten Winkler, kwinkler{at}hypKNOWsys{d0t}org
   */
  public boolean equals(Instance inst) {
    ...
    return this.equalHeaders(inst) && this.equalValues(inst);
