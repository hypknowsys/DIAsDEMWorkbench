 ...
 *
 * This class defines the [0; 1] normalized extended Jaccard, or Tanimoto,
 * distance between two instances. A distance of 0 indicates that pFirst
 * and pSecond are very similar, but not necessarily equal instances.
 * Extended Jaccard distance measure is defined as 1 - extended Jaccard
 * similarity. During the computation, attributes that are class attributes
 * (i.e., target attributes) or that are not of type weka.attribute.NUMERIC
 * are ignored. Class SparseInstance is supported.
 *
 * @version 0.1, 22 November 2004
 * @author Karsten Winkler, kwinkler{at}hypKNOWsys{d0t}org
 */

public class ExtendedJaccardDistanceMeasure extends AbstractDistanceMeasure {
  ...
  private transient double TmpExtJaccardDistance = 0.0d;
   ...
   * DistanceMeasure.OPTION_EXTENDED_JACCARD_DISTANCE) as a string.
   *
  ...
  public String getOptionArguments() {
    ...
    return DistanceMeasure.OPTION_EXTENDED_JACCARD_DISTANCE;
   ...
   * @return the extended Jaccard distance between the two given instances;
  ...
  InstancesMetadata pInstancesMetadata) {
    ...
    if (TmpScalarProduct > 0) {
      TmpExtJaccardDistance = 1.0d - (TmpScalarProduct
      / (TmpFirstLength + TmpSecondLength - TmpScalarProduct));
      if (TmpExtJaccardDistance > 1.0d) {
        TmpExtJaccardDistance = 1.0d;  // round TmpExtJaccardDistance to 1.0
      }
      else if (TmpExtJaccardDistance < 0.0d) {
        TmpExtJaccardDistance = 0.0d;  // round TmpExtJaccardDistance to 0.0
      }
    }
    else if (Utils.eq(TmpFirstLength, 0) && Utils.eq(TmpSecondLength, 0)) {
      TmpExtJaccardDistance = 0.0d;  // both instances are completely empty
    }
    else {
      TmpExtJaccardDistance = 1.0d;  // only one instance is completely empty
    }
    
    return TmpExtJaccardDistance;
  ...
  public static void main(String pOptions[]) {
    ...
    DistanceMeasure distanceMeasure = new ExtendedJaccardDistanceMeasure();
    
    System.out.println("extended Jaccard distance((2, 3); (2, 3)) = "
    + distanceMeasure.computeDistance(instance1, instance1,
    instancesMetadata));
    System.out.println("extended Jaccard distance((2, 3); (4, 6)) = "
    + distanceMeasure.computeDistance(instance1, instance2,
    instancesMetadata));
    System.out.println("extended Jaccard distance((2, 3); (-2, -3)) = "
    + distanceMeasure.computeDistance(instance1, instance3,
    instancesMetadata));
    System.out.println("extended Jaccard distance((2, 3); (3, 2)) = "
    + distanceMeasure.computeDistance(instance1, instance4,
    instancesMetadata));
    System.out.println("extended Jaccard distance((4, 6); (3, 2)) = "
    + distanceMeasure.computeDistance(instance2, instance4,
    instancesMetadata));
    System.out.println("extended Jaccard distance((3.1, 2.1); (3, 2)) = "
    + distanceMeasure.computeDistance(instance5, instance4,
    instancesMetadata));
    System.out.println("extended Jaccard distance((0, 2); (4, 6)) = "
