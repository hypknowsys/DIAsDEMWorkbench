 ...
 *
 * This class defines the [0; 1] normalized cosine distance between two
 * instances. A distance of 0 indicates that pFirst and pSecond are very
 * similar, but not necessarily equal instances. Cosine distance measure is
 * defined as 1 - cosine similarity. During the computation, attributes that
 * are class attributes (i.e., target attributes) or that are not of type
 * weka.attribute.NUMERIC are ignored. Class SparseInstance is supported.
 *
 * @version 0.1, 6 November 2003
 * @author Karsten Winkler, kwinkler{at}hypKNOWsys{d0t}org
 */

public class CosineDistanceMeasure extends AbstractDistanceMeasure {
  ...
  private transient double TmpCosineDistance = 0.0d;
   ...
   * DistanceMeasure.OPTION_COSINE_DISTANCE) as a string.
   *
  ...
  public String getOptionArguments() {
    ...
    return DistanceMeasure.OPTION_COSINE_DISTANCE;
  ...
  /**
   * Computes the [0; 1] normalized cosine distance between two instances. A
   * distance of 0 indicates that pFirst and pSecond are very similar, but
   * not necessarily equal instances. Cosine distance measure is defined as
   * 1 - cosine similarity. During the computation, attributes that are
   * class attributes (i.e., target attributes) or that are not of type
   * weka.attribute.NUMERIC are ignored. Class SparseInstance is supported.
   *
   ...
   * @return the cosine distance between the two given instances;
  ...
  InstancesMetadata pInstancesMetadata) {
    ...
    if (TmpScalarProduct > 0) {
      TmpCosineDistance = 1.0d - (TmpScalarProduct
      / (Math.sqrt(TmpFirstLength) * Math.sqrt(TmpSecondLength)));
    }
    else if (TmpScalarProduct < 0) {
      TmpCosineDistance = 1.0d + (TmpScalarProduct
      / (Math.sqrt(TmpFirstLength) * Math.sqrt(TmpSecondLength)));
    }
    else if (Utils.eq(TmpFirstLength, 0) &&  Utils.eq(TmpSecondLength, 0)) {
      TmpCosineDistance = 0.0d;  // both instances are completely empty
    }
    else {
      TmpCosineDistance = 1.0d;  // only one instance is completely empty
    }
    
    if (TmpCosineDistance > 1.0d) {
      TmpCosineDistance = 1.0d;  // round TmpCosineDistance to 1.0
    }
    else if (TmpCosineDistance < 0.0d) {
      TmpCosineDistance = 0.0d;  // round TmpCosineDistance to 0.0
    }
    
    return TmpCosineDistance;
  ...
  public static void main(String pOptions[]) {
    ...
    System.out.println("cosine distance((2, 3); (2, 3)) = " + distanceMeasure
    .computeDistance(instance1, instance1, instancesMetadata));
    System.out.println("cosine distance((2, 3); (4, 6)) = " + distanceMeasure
    .computeDistance(instance1, instance2, instancesMetadata));
    System.out.println("cosine distance((2, 3); (-2, -3)) = "
    + distanceMeasure.computeDistance(instance1, instance3,
    instancesMetadata));
    System.out.println("cosine distance((2, 3); (3, 2)) = " + distanceMeasure
    .computeDistance(instance1, instance4, instancesMetadata));
    System.out.println("cosine distance((4, 6); (3, 2)) = " + distanceMeasure
    .computeDistance(instance2, instance4, instancesMetadata));
    System.out.println("cosine distance((3.1, 2.1); (3, 2)) = "
    + distanceMeasure.computeDistance(instance5, instance4,
    instancesMetadata));
    System.out.println("cosine distance((0, 2); (4, 6)) = " + distanceMeasure
    .computeDistance(instance6, instance2, instancesMetadata));
