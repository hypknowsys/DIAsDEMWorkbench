...
import org.hypknowsys.algorithms.core.Range;
import org.hypknowsys.algorithms.core.Utils;
import org.hypknowsys.algorithms.filters.Filter;
import org.hypknowsys.algorithms.filters.unsupervised.attribute.Remove;

/**
 * Based on weka.clusterers.ClusterEvaluation, Revision 1.19:
...
public class ClusterEvaluation {
  ...
  protected StringBuffer m_clusteringResults;
   ...
   * dataset */
  protected double [] m_clusterAssignments;
  
  /** will hold the mapping of classes to clusters (for class based
   * evaluation) */
  protected int [] m_classToCluster = null;
   ...
   *
   * @return a description of the class as a string
   */
...
//    try {
      if (AbstractOptionHandler.optionsContainOptionName('h', options)) {
        throw  new Exception("Help requested.");
      }
      
      // Get basic options (options the same for all clusterers
      //printClusterAssignments = Utils.getFlag('p', options);
      objectInputFileName = AbstractOptionHandler.getOption('l', options);
      objectOutputFileName = AbstractOptionHandler.getOption('d', options);
      trainFileName = AbstractOptionHandler.getOption('t', options);
      testFileName = AbstractOptionHandler.getOption('T', options);
      
      // Check -p option
      try {
        attributeRangeString = AbstractOptionHandler.getOption('p', options);
      }
      catch (Exception e) {
      ...
      seedString = AbstractOptionHandler.getOption('s', options);
      
      if (seedString.length() != 0) {
        seed = Integer.parseInt(seedString);
      }
      
      foldsString = AbstractOptionHandler.getOption('x', options);
...
//    }
    ...
    try {
      if (trainFileName.length() != 0) {
        ...
        String classString = AbstractOptionHandler.getOption('c',options);
  ...
  private static String makeOptionString(Clusterer clusterer) {
    ...
    if (clusterer instanceof OptionHandler) {
      ...
      Enumeration enumeration = ((OptionHandler)clusterer).listOptions();
